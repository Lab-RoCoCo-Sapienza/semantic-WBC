    ...
    def process_triggers(self, ctrl_data):
        ...
        for event in ctrl_data["keyboard_event"]:
            if event["type"] == "keyboard" and not event["pressed"]:  # trigger when key is released
                key_name = event["name"]
                command = self.triggers.get(key_name, None)
                # Backward compatibility: old configs mostly use lowercase letters.
                if command is None and isinstance(key_name, str) and len(key_name) == 1 and key_name.isalpha():
                    command = self.triggers.get(key_name.lower(), None)
