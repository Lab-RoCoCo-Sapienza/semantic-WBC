...
class g1_locomimic_beyondmimic(G1RlLocoMimicPipelineCfg):
    ...
    ctrl: list[KeyboardCtrlCfg | JoystickCtrlCfg] = [
        KeyboardCtrlCfg(
            triggers={
                "i": "[SIM_REBORN]",      # Resetta il robot se cade
                "o": "[SHUTDOWN]",        # Chiude tutto
                "l": "[POLICY_LOCO]",     # L sta per LOCO (Camminata)
                "m": "[POLICY_MIMIC]",    # M sta per MIMIC (Azione/Danza)
                "n": "[POLICY_SWITCH],NEXT", # N sta per NEXT (Prossima mossa)
                "b": "[POLICY_SWITCH],LAST", # B sta per BACK (Mossa precedente)
            }
        ),
        # JoystickCtrlCfg(
    ...
    mimic_policies: list[G1BeyondMimicPolicyCfg] = [
        ...
        G1BeyondMimicPolicyCfg(policy_name="RaiseHandSideWalk", without_state_estimator=False, max_timestep=550),
    ]
