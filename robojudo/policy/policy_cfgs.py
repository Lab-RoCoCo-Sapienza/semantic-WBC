...
class BeyondMimicPolicyCfg(PolicyCfg):
    ...
    policy_name: str
    max_timestep: int = Field(
        default=-1,
        description=(
            "If > 0, stop after this many policy steps. "
            "If <= 0 and auto_motion_length=True (requires use_motion_from_model=True), "
            "estimate completion online from ONNX-emitted reference motion stability (often: hold last frame)."
        ),
    )
    start_timestep: int = 0

    auto_motion_length: bool = False
    motion_length_stable_steps: int = 5
    motion_length_pos_eps: float = 1e-4
    motion_length_min_active_steps: int = 30
    play_speed: float = Field(
        default=1.0,
        description=(
            "Motion timeline speedup applied per env step (timestep += play_speed). "
            "Decoupled from `segment_ratio` (which is a wall-clock duration multiplier). "
            "Use this to align motion BPM with the music; default 1.0 = native speed."
        ),
    )
    gui_fake_progress_horizon_steps: int = Field(
        default=900,
        description=(
            "UI-only progress horizon when `max_timestep <= 0`. Provides a monotonic fake timeline "
            "for HUD bars (ONNX exports usually don't include a true duration)."
        ),
    )
    ...
    locked_joint_names: list[str] = Field(
        default_factory=list,
        description=(
            "Action-space joint names forced to zero delta after smoothing (PD holds default_pos), "
            "e.g. waist DOFs when the hardware waist is locked. With use_motion_from_model, ref "
            "motion pos/vel in the observation are clamped for the same indices."
        ),
    )
    ...
    def check_modelmeta(self):
        ...
        if self.auto_motion_length:
            if not self.use_motion_from_model:
                raise ValueError("auto_motion_length requires use_motion_from_model=True")
            if self.max_timestep > 0:
                raise ValueError("auto_motion_length is incompatible with max_timestep > 0")
